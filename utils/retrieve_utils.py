...
def retrieve_wrapup(pl_module):
    ...
    if _config['eval_method'] == 'matmul':
        ...
        for idx in range(buffer['image_emb'].size(0)):
            ...
            _, _ , cosine_matrix[idx] = mmpe_loss(
        ...
        assert mm_query == 'multiplication'
        cosine_matrix = buffer['image_emb'] @ buffer['text_emb'].t()
    else:
        raise f"Method {_config['eval_method']} is not supported for evaluation."

    cosine_matrix = cosine_matrix.detach().cpu().numpy()
    i2t = rank(cosine_matrix, n_query)
    t2i = rank(np.transpose(cosine_matrix), n_query)
    ...
    if torch.distributed.get_rank() == 0:
        ...
        if mm_query is None:
            np.save(os.path.join(expt_path, f"image_to_{target}_score_{test_split}"), cosine_matrix)
            ...
            np.save(os.path.join(expt_path, f"image_{source[1]}_to_{target}_score_{test_split}"), cosine_matrix)
            np.save(os.path.join(expt_path, f"image_logsigma_{test_split}"), buffer['image_logsig'].detach().cpu().numpy())
