...
import os, json, tqdm,pprint
import collections, pathlib, warnings
...
class CLIPImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(c_data, np.ndarray):
            image = Image.fromarray(np.moveaxis(c_data, 0, -1))
        elif os.path.isfile(c_data):
            image = Image.open(c_data)
        else:
            raise ValueError(f"Unsupported data type: {type(c_data)}.")
